...
public class HelperRendererView {
    ...
    public static void handleItem(ItemStack stack) {
        if (!isActive) return;
        HelperRendererView.getCachedPart(stack).flatMap(
                part -> HelperRendererView.getRenderPreviews().stream()
    ...
    public static boolean handleItemValid(ItemStack stack) {
        return HelperRendererView.getCachedPart(stack).flatMap(
                part -> HelperRendererView.getRenderPreviews().stream()
                        .filter(preview -> preview.validItemClass().stream().anyMatch(clazz -> clazz.isInstance(part)))
                        .findFirst())
                .isPresent();
